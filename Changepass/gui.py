"""
gui.py — PyQt6 MainWindow và stylesheet (Changepass)
"""
...
QPushButton#btn_start:hover    { background: #9e3900; }
QPushButton#btn_start:disabled { background: #2a1a1a; color: #556; }
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Riot  —  Change Password")
    ...
    def _build_toolbar(self, parent_layout):
        ...
        logo_box.setStyleSheet("background:#c44a00; border:1px solid #e05a10;")
    ...
    def _build_right(self, parent_layout):
        ...
        t = QLabel("Danh sach doi mat khau")
    ...
    def _on_start(self):
        ...
        self.status_bar.setText("Dang doi mat khau...")
        core.trigger_run()
    ...
    def _export_txt(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Luu file", "changed_accounts.txt", "Text Files (*.txt)"
        )
        if not path:
            return
        lines = []
        for e in core.acct_log:
            status = e.get("status", "?")
            new_pw = e.get("new_password", "")
            lines.append(f"{e['username']}|{new_pw}|{status}")
    ...
    def _refresh_table(self):
        ...
        for row, e in enumerate(core.acct_log):
            ...
            self.table.setItem(row, 1, cell(e.get("old_password", "")))
            self.table.setItem(row, 2, cell(e.get("new_password", ""), center))
