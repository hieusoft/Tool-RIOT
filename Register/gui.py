"""
gui.py — PyQt6 MainWindow and stylesheet
"""
...
QPushButton#btn_start:hover    { background: #12864e; }
QPushButton#btn_start:disabled { background: #1a3a2a; color: #556; }
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Riot  —  Admin Panel")
    ...
    def _build_toolbar(self, parent_layout):
        ...
        logo_box.setStyleSheet("background:#1f5aff; border:1px solid #3b70fc;")
        ...
        self.btn_export.setStyleSheet(
            "QPushButton#btn_export { background:#1a3050; color:#7ab0ff; border:1px solid #2f5080; "
    ...
    def _build_right(self, parent_layout):
        ...
        self.stat_done    = self._stat_card(cards_h, "HOAN THANH", "0", "#4ae98c", last=True)
    ...
    def _on_start(self):
        ...
        self.status_bar.setText("Dang chay kich ban...")
        core.trigger_run()
    ...
    def _export_txt(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Luu file", "accounts.txt", "Text Files (*.txt)"
        )
        if not path:
            return
        lines = []
        for e in core.acct_log:
            lines.append(f"{e['username']}|{e['email']}|{e['password']}")
    ...
    def _refresh_table(self):
        ...
        for row, e in enumerate(core.acct_log):
            ...
            self.table.setItem(row, 0, cell(e["username"]))
            self.table.setItem(row, 1, cell(e["email"]))
            self.table.setItem(row, 2, cell(e["password"], center))
            self.table.setItem(row, 3, cell(str(e["session_id"])[:22] + "..."))
            self.table.setItem(row, 4, cell(status, center))

    def _refresh_stats(self):
        self.stat_conn.setText(str(len(core.sessions)))
        self.stat_running.setText(str(sum(1 for e in core.acct_log if "chay" in e.get("status", ""))))
